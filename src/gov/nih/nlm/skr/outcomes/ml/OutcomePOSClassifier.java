...
//import gov.nih.nlm.features.span.NGramFeatures;
//import gov.nih.nlm.features.span.NGramFeatures.BigramFeature;
//import gov.nih.nlm.features.span.NGramFeatures.UnigramFeature;
 ...
 *
 */
public class OutcomePOSClassifier extends OutcomeOracle {
	private static final Log log = new Log(OutcomePOSClassifier.class);
	 ...
	 * {@link OutcomeOracle}.
	 */  
	public OutcomePOSClassifier() {
		oracle = OutcomeOracle.multiclassMetaData("goldPOSOutcome");
	}

	public boolean isKnown(final DocumentInstance request) {
		return request.getMetaDataMap().containsKey("goldPOSOutcome");
	}

	/**
	 * Listener for classification results.
	 */
	public static abstract class OutcomePOSClassificationListener {
	...
	private final List<TrainingListener<DocumentInstance>> trainingListeners =
			new ArrayList<>();
	private final List<OutcomePOSClassificationListener> classificationListeners =
			new ArrayList<>();
	...
	public Pair<String,Double> classify2(final DocumentInstance request) {
		...
		if (binary) {
			...
			for (final OutcomePOSClassificationListener listener : classificationListeners) {
	...
	public void train(final Iterable<DocumentInstance> requests, List<String> featuresToUse,
			final boolean quiet) {
		final Classifier<DocumentInstance> classifier = getClassifier();
		classifier.setFeatureValueLog(Config.get(OutcomePOSClassifier.class, "log").toPlace());
		...
		for (final DocumentInstance request : requests) {
			fe.sample(request);
			String type = request.getMetaData("goldPOSOutcome");
			int nc = 0;
	...
	public void test(final Iterable<DocumentInstance> requests,
		...
		for (final DocumentInstance request : requests) {
			...
			//		    log.info("Req: " + request.toString() + "\t" + gold + "|" + guess);
			numRequests++;
		}
	}
	 ...
	 * <code>MachineRequestClassifier</code>.
	 */
	public void addClassificationListener(final OutcomePOSClassificationListener listener) {
	...
	protected Classifier<DocumentInstance> getClassifier() {
		if (svm == null) {
			...
			final Place modelFile =
					Config.get(OutcomePOSClassifier.class, "model").toPlace();
	...
	public static List<Feature<DocumentInstance,?>> NiuFeatures() {
		List<Feature<DocumentInstance,?>> features = new ArrayList<Feature<DocumentInstance,?>>();
		final OverallOutcomeFeatures ooFeatures = new OverallOutcomeFeatures();
		features.add((Feature<DocumentInstance,?>)ooFeatures.new ContextUnigramFeature(OverallOutcomeFeatures.Type.PORTER));
		features.add((Feature<DocumentInstance,?>)ooFeatures.new ContextBigramFeature(OverallOutcomeFeatures.Type.PORTER));
		features.add((Feature<DocumentInstance,?>)ooFeatures.new MorePOSFeature(4));
		features.add((Feature<DocumentInstance,?>)ooFeatures.new MoreNEGFeature(4));
		features.add((Feature<DocumentInstance,?>)ooFeatures.new LessPOSFeature(4));
		features.add((Feature<DocumentInstance,?>)ooFeatures.new LessNEGFeature(4));
		features.add((Feature<DocumentInstance,?>)ooFeatures.new ChangePhraseUnigramFeature(OverallOutcomeFeatures.Type.PORTER));
		features.add((Feature<DocumentInstance,?>)ooFeatures.new NegationPhraseFeature(OverallOutcomeFeatures.Type.PORTER));

		for (String s: SemRepConstants.SEMREP_ENTITY_TYPES) {
			features.add((Feature<DocumentInstance,?>)ooFeatures.new SemTypePresentFeature("Semtype(" + s +")", s));
		}
		return features;
	}
